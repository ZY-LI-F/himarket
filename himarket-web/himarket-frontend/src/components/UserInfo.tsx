...
export function UserInfo() {
  ...
  const menuItems = [
    {
      ...
      label: (
        <div>
          <div className="text-base font-semibold text-claude-neutral-900">
            {userInfo?.displayName}
          </div>
          {userInfo?.email && (
            <div className="mt-0.5 text-xs text-claude-neutral-500">
              {userInfo.email}
            </div>
          )}
        </div>
  ...
  if (userInfo) {
    return (
      ...
      >
        <div className="flex cursor-pointer items-center space-x-2 rounded-claude-full px-2 py-1 transition-all duration-claude-base ease-claude hover:bg-colorPrimaryBgHover">
          {userInfo.avatar ? (
            <Avatar src={userInfo.avatar} size="default" />
          ) : (
            <Avatar
              size="default"
              className="bg-colorPrimaryBgHover text-colorPrimary font-medium"
            >
  ...
  return (
    <Button
      ...
      type="text"
      className="rounded-claude-full bg-colorPrimary text-claude-neutral-50 border-none hover:opacity-90 hover:!bg-colorPrimary"
    >
      {t("login")}
    </Button>
  );
